...
/* USER CODE BEGIN PV */
static uint32_t ADC_au32Channel[ADC_enTotalSensors] = ADC_CHANNEL_TABLE;
/* USER CODE END PV */
...
/* Exported functions --------------------------------------------------------*/
/* USER CODE BEGIN EF */
uint32_t ADC_u32GetResolution(void)
{
	uint32_t resolution = 0U;
	switch(hadc.Init.Resolution)
	{
	case ADC_RESOLUTION_12B:
		resolution = (uint32_t)ADC_RESOLUTION_12BITS;
		break;
	case ADC_RESOLUTION_10B:
		resolution = (uint32_t)ADC_RESOLUTION_10BITS;
		break;

	case ADC_RESOLUTION_8B:
		resolution = (uint32_t)ADC_RESOLUTION_8BITS;
		break;
    
	case ADC_RESOLUTION_6B:
		resolution = (uint32_t)ADC_RESOLUTION_6BITS;
		break;
	}
	return resolution;
}

uint32_t ADC_u32ReadChannel(uint16_t channel)
{
	return ADC_ReadChannels(ADC_au32Channel[channel]);
}
